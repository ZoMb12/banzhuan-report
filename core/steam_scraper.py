        ...
        try:
            ...
            if not matched:
                ...
                _last_error_context = {
                    ...
                    "steam_url": steam_url[:100],
                    "buff_props": props,
                    "available_buckets": [b.get("localized_name") for b in buckets[:20]],
...
def get_steam_market_data_batch(
    ...
    with sync_playwright() as p:
        ...
        _try_add_cookies(context, _load_cookies_from(config.STEAM_COOKIE_PATH), "Steam")
        _try_add_cookies(context, _load_cookies_from(config.COOKIE_PATH), "BUFF")
        page = context.new_page()
                ...
                if not matched:
                    _batch_errors[item_id] = {
                        ...
                        "context": {
                            ...
                            "item_id": item_id, "buff_item_name": buff_item_name,
                            "buff_props": props,
                            "available_buckets": [b.get("localized_name") for b in buckets[:20]],
                        },
                    }
...
def retry_steam_failed_items(failed_items: List[dict]) -> dict:
    ...
    for fi in failed_items:
        base_name = fi.get("base_skin_name") or _get_base_skin_name(fi.get("buff_item_name", ""))
